...
# %2F #links_comentarios_fb <- str_remove_all(links_comentarios_fb,"/")

# Usamos RSelenium
rd <- rsDriver(browser = "firefox", port = 7676L)
ffd <- rd$client
...
leer_fb <- function (x){
  tryCatch({
    ffd$navigate(x)
    Sys.sleep(2)
    mascomentarios <- ffd$findElements("class name", "_2pir")
    n <- length(mascomentarios)
    if(n > 0){
      for (i in 1:n){
        print(i)
...
leer_fb <- function (x){
  tryCatch({
    ...
    fb <-  ffd$getPageSource()[[1]] %>% read_html()
    tibble(
      post_img =  html_nodes(fb, "._46-i.img") %>% html_attr("src"),
      bajado = Sys.time(),
      nota = x
    )
  }, error = function(e){
...
leer_fb <- function (x){
  tryCatch({
    ...
    fb <-  ffd$getPageSource()[[1]] %>% read_html()
    tibble(
      post_res_img =  html_elements(fb, ".scaledImageFitWidth.img") %>% html_attr("src"),
      bajado = Sys.time(),
      nota = x
    )
  }, error = function(e){
...
leer_fb <- function (x){
  tryCatch({
    ...
    tibble(
      #post_img =  html_nodes(fb, "._46-i.img,.scaledImageFitWidth.img") %>% html_attr("src"),
      post_img = html_nodes(fb, "._30o4,._46-i.img,.scaledImageFitWidth.img") %>% as.character(),
      bajado = Sys.time(),
      nota = x
    )
  }, error = function(e){
